    ...
    <aside className={`min-h-max min-w-[100vw] absolute sm:static sm:translate-x-0 ${!expanded && "-translate-x-full"} sm:min-w-min transition-all`}>
      <nav className="h-full flex flex-col bg-white border-r shadow-sm">
        <div className="p-4 pb-2 flex justify-between items-center">
          <h2 className={`font-serif font-extrabold text-primary-dark transition-all overflow-hidden ${expanded ? "w-32" : "w-0"}`}>Dashboard</h2>
        ...
        <ul className="flex-1 px-3">
          ...
          <SideBarItems
            ...
            text="Update Fleet"
            onClick={() => handleItemClick('UpdateFleet')}
            selected={selected === 'UpdateFleet'}
          />
          <hr className="my-3" />
          <SideBarItems
            expanded={expanded}
